...
function runSimulation(withCompaction: boolean) {
    const memorySize = Number((document.getElementById("memorySize") as HTMLInputElement).value);
    const osSize = Number((document.getElementById("osSize") as HTMLInputElement).value);
    const text = (document.getElementById("jobs") as HTMLTextAreaElement).value.trim();

    const jobs: Job[] = text.split("\n").map((line, i) => {
...
function displayResults(jobs: Job[], withCompaction: boolean) {
